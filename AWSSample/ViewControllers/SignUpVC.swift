...
final class SignUpVC: UIViewController {
    ...
    @IBAction func onCreateAccount(_ sender: Any) {
        ...
        CognitoUserPoolManager.sharedInstance.signup(username: username, password: password, emailAddress: emailAddress) { error, user in
    ...
    fileprivate func requestConfirmCode(_ user: AWSCognitoIdentityUser) {
        UIAlertController.alertTextField(title: "Confirmation", message: "Type the 6-digit code that has been sent to your email.") { texts in
            ...
            CognitoUserPoolManager.sharedInstance.confirmSignup(user: user, confirmationCode: confirmationCode) { error in
                DispatchQueue.main.async { [weak self] in
                    guard let `self` = self else { return }
                    if let error = error {
